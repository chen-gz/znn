...
| 性能指标 | Zig (单线程) | Zig (多线程) | Zig (CBLAS/AMX CPU) 🚀 | Zig (Metal GPU) ⚡ | Zig (MLX - 全托管) ⚠️ | Zig (MLX - 纯GPU常驻) 🚀 | Python (MLX - 原生) 🐍 | PyTorch (MPS GPU) | 对比与说明 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
| **平均每 Epoch 耗时** | **~4.45 秒** | **~2.60 秒** | **~0.25 秒** | **~2.50 秒** | **~4.46 秒** | **~2.55 秒** | **~0.81 秒** | **~2.75 秒** | **CBLAS / AMX (0.25s) 最快**；纯 GPU 常驻版本消除了频繁拷贝开销，显著加速。 |
| **15 Epochs 总耗时** | ~67 秒 | ~39 秒 | **~3.7 秒** | **~37.5 秒** | **~66.9 秒** | **~38.3 秒** | **~12.2 秒** | **~41.3 秒** | 小模型下 CPU/AMX 是绝对主力，GPU 调度开销明显。 |
...
## 六、 超大网络（4层 MLP: 784 -> 2048 -> 2048 -> 1024 -> 10）对比数据

为了进一步探索更大模型规模下的计算性能，我们测试了包含约 **800万** 参数量的超大型 4 层全连接网络。在相同的硬件环境下，测试数据对比如下：

| 性能指标 | Zig (CBLAS/AMX CPU) 🚀 | Zig (MLX - 全托管) ⚠️ | Zig (MLX - 纯GPU常驻) 🚀 | Python (MLX - 原生) 🐍 | PyTorch (MPS GPU) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **底层设备与核心** | **苹果 Accelerate (AMX)** | **MLX C API (GPU)** | **MLX C API (GPU)** | **MLX Python (GPU)** | **PyTorch MPS (GPU)** |
| **平均每 Epoch 耗时** | **~31.44 秒** | **~55.73 秒** | **~12.19 秒** | **~5.72 秒** | **~6.07 秒** |
| **相比标准模型的减速比** | **125x 变慢** (0.25s -> 31.44s) | **12x 变慢** (4.46s -> 55.73s) | **4.8x 变慢** (2.55s -> 12.19s) | **7.0x 变慢** (0.81s -> 5.72s) | **2.2x 变慢** (2.75s -> 6.07s) |

### 纯 GPU 常驻与普通全托管的区别：
1. **纯 GPU 常驻 (mlx_zig_pure) 的性能飞跃**：
   - 我们在 `mlx_zig_pure` 目录实现了一个**零 CPU 拷贝常驻显存**的后端。在这个后端下，Tensor 和 Gradients 的数据全程只留在显存（`mlx_array` 指针的形式）中进行前向、反向及 SGD 更新，不再在每个算子结束后调用 evaluation 和 `@memcpy`。
   - 这解除了数据传输的瓶颈，使得超大模型的单 Epoch 时间从 **55.73s** 缩短至 **12.19s**，实现了 **4.6 倍的加速**，并且比 AMX CPU（**31.44s**）快了 **2.6 倍**。
2. **为什么与原生 Python MLX 还有约 2 倍的时间差？**
   - **单 JIT 会话融合边界 (Session Fusion Boundary)**：在原生 MLX 中，整个 Batch 的前向、交叉熵、反向和更新被集成在一个大 JIT 函数中整体编译。而我们在 Zig 端，为了配合 Softmax Cross Entropy 逻辑，在 Logits 层将数据临时拉回 CPU 运行，将大 JIT 图拆分成了“前向”与“反向/更新”两个独立派发步骤。这阻止了 MLX 对整个算子链进行跨前向-反向的极限融合，产生了一定程度的调度延迟。
