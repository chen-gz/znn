...
pub fn generateHtmlReport(graph: *Graph, allocator: std.mem.Allocator) ![]const u8 {
    ...
    try html_buf.appendSlice(allocator,
        ...
        \\  }
        \\  if (k.endsWith('.layers') || k === 'layers' || k.includes('decoder')) {
        \\    return { formula: 'x_L = \\\\text{DecoderStack}(x_0) = (\\\\text{Layer}_L \\\\circ \\\\dots \\\\circ \\\\text{Layer}_1)(x_0) \\\\rightarrow [B, T, D]', source: 'INFERRED' };
        \\  }
        \\  if (k.includes('wte')) return { formula: 'y = \\\\text{Embedding}(\\\\text{TokenIDs}; W_e \\\\in \\\\mathbb{R}^{V \\\\times D}) \\\\rightarrow [B, T, D]', source: 'INFERRED' };
        \\  if (k.includes('wpe')) return { formula: 'y = \\\\text{Embedding}(\\\\text{PosIDs}; W_p \\\\in \\\\mathbb{R}^{T_{max} \\\\times D}) \\\\rightarrow [B, T, D]', source: 'INFERRED' };
        \\  if (k.includes('embeddings_sum') || (k.includes('embeddings') && k.includes('add'))) return { formula: 'x_0 = \\\\text{wte}(\\\\text{tokens}) + \\\\text{wpe}(\\\\text{positions}) \\\\rightarrow [B, T, D]', source: 'INFERRED' };
        ...
        \\  const isRootModule = (key === 'gpt' || key === 'model' || !key.includes('.'));
        ...
        \\
        \\  if (isRootModule) {
        \\    const inputNode = NODES_DATA.find(n => n.kind === 'Input' && (n.name.includes('token') || n.name.includes('input')));
        \\    if (inputNode) modInpShape = `${inputNode.shape} (Token IDs)`;
        \\    const outputNode = NODES_DATA.find(n => n.name.includes('logits') || n.name.includes('output'));
        \\    if (outputNode) modOutShape = `${outputNode.shape} (Logits)`;
        \\  }
        \\
        ...
        \\    else if (key.includes('lm_head')) modOutShape = '[2, 16, 256] (Logits)';
        \\  }
        \\
        \\  let ioBannerHtml = `
        ...
        \\
        \\  // Leaf module: render directly on a single row without nested folder
        \\  if (childKeys.length === 0 && !isRoot) {
        \\    const opInfo = getNodeOpType(prefix);
        \\    return `
        \\      <div class="tb-node-card" onclick="openInspector('${prefix}')" title="Click to inspect ${prefix} inputs, parameters & outputs" style="margin-bottom: 8px;">
