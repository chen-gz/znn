...
pub const AdamOptimizer = struct {
    allocator: std.mem.Allocator,
    states: std.ArrayList(ParamState),
    t: usize = 0,
    lr: f32,
    beta1: f32 = 0.9,
    beta2: f32 = 0.999,
    ...
    pub fn init(allocator: std.mem.Allocator, model: anytype, lr: f32, beta1: f32, beta2: f32, eps: f32) !AdamOptimizer {
        var states: std.ArrayList(ParamState) = .empty;
        errdefer {
            for (states.items) |s| {
                allocator.free(s.m);
                allocator.free(s.v);
        ...
        try collectModelParams(model, &states, allocator);
    ...
    pub fn deinit(self: *AdamOptimizer) void {
        for (self.states.items) |s| {
            self.allocator.free(s.m);
            self.allocator.free(s.v);
        }
    ...
    pub fn step(self: *AdamOptimizer) void {
        ...
        const step_size = self.lr * @sqrt(bias_correction2) / bias_correction1;

        for (self.states.items) |state| {
            const w = state.tensor_ptr.data;
            const dw = state.tensor_ptr.grad;
            for (w, dw, state.m, state.v) |*weight, grad, *m_val, *v_val| {
                m_val.* = self.beta1 * m_val.* + (1.0 - self.beta1) * grad;
                v_val.* = self.beta2 * v_val.* + (1.0 - self.beta2) * (grad * grad);
                weight.* -= step_size * m_val.* / (@sqrt(v_val.*) + self.eps);
            }
        }
    }

    fn collectModelParams(model: anytype, states: *std.ArrayList(ParamState), allocator: std.mem.Allocator) anyerror!void {
        ...
        inline for (info.@"struct".fields) |field| {
            ...
            const field_info = @typeInfo(FieldType);
            if (FieldType == *Tensor) {
                const t_ptr = @field(model, field.name);
                if (t_ptr.requires_grad) {
                    const len = t_ptr.data.len;
                ...
                try collectModelParams(&@field(model, field.name), states, allocator);
            } else if (field_info == .@"array") {
                const elem_info = @typeInfo(field_info.@"array".child);
                if (elem_info == .@"struct") {
                    for (&@field(model, field.name)) |*item| {
                        try collectModelParams(item, states, allocator);
                    }
                }
            }
        }
